...
def numpy_array_yaml_representer(dumper: yaml.Dumper, data: np.ndarray | torch.Tensor) -> Any:
    r"""Return the yaml representation of a numpy array or torch
    tensor."""
    if data.ndim == 0:
        scalar = data.item()
...
def numpy_datetime_yaml_representer(dumper: yaml.Dumper, data: np.datetime64) -> Any:
    r"""Return the yaml representation of a numpy datetime64."""
    return dumper.represent_str(str(data))


def numpy_float_yaml_representer(dumper: yaml.Dumper, data: np.floating) -> Any:
    r"""Return the yaml representation of a numpy float64/float32."""
    return dumper.represent_float(float(data))


def numpy_int_yaml_representer(dumper: yaml.Dumper, data: np.integer) -> Any:
    r"""Return the yaml representation of a numpy int64/int32."""
...
yaml.Dumper.add_representer(torch.Tensor, numpy_array_yaml_representer)
...
def load_text(path: Path) -> str:
    ...
    ```pycon
    >>> from pathlib import Path
    >>> from iden.utils.io import load_text
...
def save_text(to_save: str, path: Path) -> None:
    ...
    ```
    """
    logger.debug(f"Saving data in text file: {path} ")
...
class TensorJSONEncoder(json.JSONEncoder):
    r"""Define a custom json encoder to work with numpy arrays and
    tensors."""
...
def load_json(path: Path) -> dict:
    ...
    ```pycon
    >>> from pathlib import Path
    >>> from iden.utils.io import load_json
...
def save_json(to_save: Any, path: Path) -> None:
    ...
    ```pycon
    >>> from pathlib import Path
    >>> from iden.utils.io import save_json
...
def save_pytorch(to_save: Any, path: Path) -> None:
    r"""Save the data in a PyTorch file.

    Args:
        to_save: Specifies the data to write in the PyTorch file.
        path: Specifies the path where to write the PyTorch file.

    Example usage:

    ```pycon
    >>> from pathlib import Path
    >>> from iden.utils.io import save_pytorch
...
def load_yaml(path: Path) -> Any:
    ...
    ```pycon
    >>> from pathlib import Path
    >>> from iden.utils.io import load_yaml
...
def save_yaml(to_save: Any, path: Path) -> None:
    r"""Save the data in a yaml file.
    ...
    ```pycon
    >>> from pathlib import Path
    >>> from iden.utils.io import save_yaml
