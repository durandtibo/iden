...
def test_json_shard_clear_is_cached(uri: str, path: Path) -> None:
    shard = JsonShard(uri=uri, path=path)
...
def test_json_shard_is_cached_false(uri: str, path: Path) -> None:
    shard = JsonShard(uri=uri, path=path)
    assert not shard.is_cached()


def test_json_shard_is_cached_true(uri: str, path: Path) -> None:
    shard = JsonShard(uri=uri, path=path)
    shard.get_data(cache=True)
    assert shard.is_cached()
