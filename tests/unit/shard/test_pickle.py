...
def test_pickle_shard_repr(uri: str, path: Path) -> None:
    assert repr(PickleShard(uri=uri, path=path)).startswith("PickleShard(")
...
def test_pickle_shard_clear_is_cached(uri: str, path: Path) -> None:
    shard = PickleShard(uri=uri, path=path)
...
def test_pickle_shard_equal_false_different_type_child(uri: str, path: Path) -> None:
    class Child(PickleShard): ...

    assert not PickleShard(uri=uri, path=path).equal(Child(uri=uri, path=path))
...
def test_pickle_shard_get_data_cache_false_not_cached(uri: str, path: Path) -> None:
    shard = PickleShard(uri=uri, path=path)
    assert not shard.is_cached()
...
def test_pickle_shard_get_data_cache_false_cached(uri: str, path: Path) -> None:
    shard = PickleShard(uri=uri, path=path)
...
def test_pickle_shard_get_data_cache_true_not_cached(uri: str, path: Path) -> None:
    shard = PickleShard(uri=uri, path=path)
    assert not shard.is_cached()
    assert objects_are_equal(shard.get_data(cache=True), {"key1": [1, 2, 3], "key2": "abc"})
    assert shard.is_cached()
...
def test_pickle_shard_get_data_cache_true_cached(uri: str, path: Path) -> None:
    shard = PickleShard(uri=uri, path=path)
    shard.get_data(cache=True)
    assert shard.is_cached()
    assert objects_are_equal(shard.get_data(cache=True), {"key1": [1, 2, 3], "key2": "abc"})
...
def test_pickle_shard_is_cached_false(uri: str, path: Path) -> None:
    shard = PickleShard(uri=uri, path=path)
    assert not shard.is_cached()


def test_pickle_shard_is_cached_true(uri: str, path: Path) -> None:
    shard = PickleShard(uri=uri, path=path)
    shard.get_data(cache=True)
    assert shard.is_cached()
