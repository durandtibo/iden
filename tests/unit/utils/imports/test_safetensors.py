...
def test_check_safetensors_with_package() -> None:
    with patch("iden.utils.imports.safetensors.is_safetensors_available", lambda: True):
        check_safetensors()
...
def test_safetensors_available_without_package() -> None:
    with patch("iden.utils.imports.safetensors.is_safetensors_available", lambda: False):
        fn = safetensors_available(my_function)
        assert fn(2) is None
